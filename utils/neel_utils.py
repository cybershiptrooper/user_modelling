...
def create_vocab_df(logit_vec, make_probs=False, full_vocab=None, model=None):
    ...
    if full_vocab is None:
        full_vocab = process_tokens(
            model.to_str_tokens(torch.arange(model.cfg.d_vocab)), model
        )
...
def get_induction_scores(model, make_plot=False, batch_size=4, ind_seq_len=200):
    ...
    if make_plot:
        imshow(
            ind_head_scores, xaxis="Head", yaxis="Layer", title="Induction Head Scores"
        )
...
def get_variable_from_caller(var_name):
    ...
    try:
        ...
        if variable is None:
            raise ValueError(
                f"The '{var_name}' variable does not exist in the calling scope."
            )
        return variable
    finally:
...
def plot_line_and_df(
    ...
    **kwargs,
    ...
    if refactor_decomp:
        ...
        y = np.concatenate(
            [y[..., :prefix], y[..., prefix::2], y[..., prefix + 1 :: 2]], axis=-1
        )
    ...
    if len(y.shape) == 1:
        df = pd.DataFrame({"0": to_numpy(y)}, index=x).sort_values(
